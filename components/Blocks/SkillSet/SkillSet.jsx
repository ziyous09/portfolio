...
export default function SkillSet() {
    // register the plugins we know are available; InertiaPlugin is a Club GreenSock
    // plugin and may not be present in all environments. We'll attempt to load it
    // dynamically below and create Draggable after that so we can optionally
    // enable inertia when available.
    ...
    useGSAP(() => {
        ...
        // Draggable - use inertia: false by default to avoid requiring the
        // Club GreenSock InertiaPlugin at build time. If you install the plugin
        // separately in the future, you can enable it and register it above.
        setActiveIndex(0);

        let draggableInstances = Draggable.create(`.${styles.circularCarousel}`, {
            type: "rotation",
            inertia: false,
            // Smoothness and UX tweaks for mobile
            ...
            onRelease: () => {
                setDragStatus(null);
                // compute active index on release (in case inertia/throw isn't used)
                try {
                    const collisionDivRect = collisionDiv.current?.getBoundingClientRect();
                    if (!collisionDivRect) return;
                    // find the box that overlaps the collision div or nearest one
                    ...
                    boxes.forEach((box, index) => {
                        ...
                        // compute distance fallback (center-to-center)
                        const cx = collisionDivRect.x + collisionDivRect.width / 2;
                        const cy = collisionDivRect.y + collisionDivRect.height / 2;
                        const bx = boxRect.x + boxRect.width / 2;
                        const by = boxRect.y + boxRect.height / 2;
                        const dist = Math.hypot(cx - bx, cy - by);
                        if (dist < minDist) {
                            minDist = dist;
                            // only set as fallback if no overlap has been found
            ...
            onThrowComplete: () => {
                // same computation as onRelease: prefer overlap, fallback to nearest
                try {
                    const collisionDivRect = collisionDiv.current?.getBoundingClientRect();
                    if (!collisionDivRect) return;
                    let newActiveIndex = null;
        ...
        // Lighten scroll animation on small screens to avoid jank
        ...
        mm.add("(max-width: 767px)", () => {
            ...
            tl.fromTo(sphere.current, { yPercent: -120 }, { yPercent: -40 });
            return () => tl?.kill?.();
        });
        mm.add("(min-width: 768px)", () => {
            ...
            tl.from(sphere.current, { yPercent: -200 });
            return () => tl?.kill?.();
        });
        ...
        return () => {
            ...
            try {
                if (draggableInstances && draggableInstances.length) {
                    draggableInstances.forEach(d => d && d.kill && d.kill());
                } else if (draggableInstances && draggableInstances.kill) {
                    // single instance
                    draggableInstances.kill();
                }
            } catch (e) {}
            // kill timeline and ScrollTriggers
            try {
                if (sphereTL && sphereTL.kill) sphereTL.kill();
    ...
    return (
        <>
            ...
            <Ticker words={['responsiveness', 'interactive', 'performance']}></Ticker>
        </>
    )
}